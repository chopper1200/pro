...
import {
  ...
  ExternalLink, Info, Trash2, Flag, Sparkles, Trophy, Award,
} from "lucide-react";
      ...
      {ex && (
        <Card className="overflow-hidden" data-testid="exercise-focus-card">
          ...
          <div className="p-4 space-y-4">
            ...
            {ex.suggestionLabel && (
              <div className="flex items-center gap-2 text-sm rounded-lg p-2.5 bg-primary/10 border border-primary/30" data-testid="overload-suggestion">
                <Sparkles className="w-4 h-4 text-primary shrink-0" />
                <span className="text-foreground">
                  Sovraccarico progressivo: <span className="font-semibold text-primary">{ex.suggestionLabel}</span>
                  <span className="text-muted-foreground"> (precompilato)</span>
                </span>
              </div>
            )}
